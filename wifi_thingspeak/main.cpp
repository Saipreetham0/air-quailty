...
const unsigned long thingSpeakInterval = 20000; // 20 seconds (ThingSpeak free limit is 15 seconds)
...
void sendToThingSpeak(float pm25, float pm10, int aqi, float temp, float hum, float vocs, float co2, int smoke) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi not connected. Skipping ThingSpeak update.");
    return;
  }

  // Set the fields for ThingSpeak (max 8 fields)
  ThingSpeak.setField(1, pm25);        // Field 1: PM2.5
  ThingSpeak.setField(2, pm10);        // Field 2: PM10
  ThingSpeak.setField(3, aqi);         // Field 3: AQI
  ThingSpeak.setField(4, temp);        // Field 4: Temperature
  ThingSpeak.setField(5, hum);         // Field 5: Humidity
  ThingSpeak.setField(6, vocs);        // Field 6: VOCs
  ThingSpeak.setField(7, co2);         // Field 7: CO2
  ThingSpeak.setField(8, smoke);       // Field 8: Smoke Level
...
void setup() {
  ...
  Serial.println("Air Quality Monitor with WiFi & ThingSpeak started");
...
void loop() {
  ...
  // === SEND DATA TO THINGSPEAK (with timing control) ===
  unsigned long currentMillis = millis();
  if (currentMillis - lastThingSpeakUpdate >= thingSpeakInterval) {
    sendToThingSpeak(pm25, pm10, aqi, temperature, humidity, vocs, co2_ppm, smokeLevel);
